...
.. note::
    ...
    新建项目时url目前只支持GitHub(使用https方法)，可以是公开仓库或私有仓库，私有仓库
    的url格式是：https://username:password@github.com/username/repo

选项 `--custom-domain/-cd` 用来自定义域名，不包含协议，如果自定义域名想要支持HTTPS，
请设置选项 `--ssl, --ssl-crt, --ssl-key` ，选择证书和密钥。

已创建的项目可以更新项目，上述共四个选项在更新时会重新渲染nginx配置，生成自定义域名的
配置文件，用户需要给自定义域名添加CNAME记录，指向程序配置文件中dn的域名。
